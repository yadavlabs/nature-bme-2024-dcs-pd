...
load(fullfile(data_path, 'stickDecompData.mat'))
...
for m = 1:SegDens:xy_len
    ...
    for i = 1:length(axs)
        ...
        plot(axs(i), stickDecompData(i).x(m,:), stickDecompData(i).y(m,:), 'Color', colors{i}, 'LineWidth', LineWidth)
        %addpoints(hs(i),plotData(i).x(m,:), plotData(i).y(m,:))
        %drawnow
        %pause(1/pns_fs)
