...
for p=1:n
    ...
    if nargin<3 || isempty(z)
        if nargin<2 || isempty(y)  % only x given
            hand(p)=plot(x(part),x(part),'color',cm(p,:),'MarkerFaceColor',cm(p,:),'LineWidth',1.5);
        else    % x and y given
                hand(p)=plot(x(part),y(part),'color',cm(p,:),'MarkerFaceColor',cm(p,:),'LineWidth',1.5);
        end
    else % x, y, and z given
            hand(p)=plot3(x(part),y(part),z(part),'color',cm(p,:),'MarkerFaceColor',cm(p,:),'LineWidth',1.5);
    end
    hold on
end
